      SUBROUTINE DREORTH (LBLAS,LCOMM,LRERR,LRWRN,NI,NVB,NULLP,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DREORTH performs a local reorthogonalization                     *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C
      ...
      DOUBLE PRECISION AMAXN,BMAXN,BMINN,EPS,REPS
      LOGICAL          GNRZD
C
      INTEGER          LBLAS(*)
      DOUBLE PRECISION BETA(*),BQ(*),BR(*),Q(*),R(*),WORK(*)
C
C==== local variables ==================================================
C
...
C
         IF ( GNRZD ) THEN
            CALL DQTBR   (NI,BQ,NI,NVB,R,NI,NVB,WORK(IWORK1),WMAX,
     &                    WORK(IWORK2),LBLAS(2),LCOMM,LRERR)
            CALL DRQALPH (NI,BR,NI,NVB,BQ,NI,NVB,WORK,LBLAS(3))
            CALL DRQALPH (NI, R,NI,NVB, Q,NI,NVB,WORK,LBLAS(3))
         ELSE
            CALL DQTBR   (NI, Q,NI,NVB,R,NI,NVB,WORK(IWORK1),WMAX,
     &                    WORK(IWORK2),LBLAS(2),LCOMM,LRERR)
            CALL DRQALPH (NI, R,NI,NVB, Q,NI,NVB,WORK,LBLAS(3))
         END IF
C
C....... (R)=(Q)*(BETA) updating .......................................
C
         CALL DRFACTR (LBLAS(4),LCOMM,LRERR,LRWRN,NI,NVB,NULLP,BETA,
