      SUBROUTINE DEIGCHK (INDXP,INDXS,LEIG,LNI,LRWRN,NNSPNT,NPEIG,NREIG,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DEIGCHK checks the computed eigenpairs                           *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    DEIGIDX,DNEIGAB,DEIGSRT,DSETLRM                                  *
C*                                                                     *
C*  - BLAS kernel:                                                     *
C*                                                                     *
C*    DCOPY                                                            *
...
C
     ...
     &                 NNSPNT,NPEIG,NREIG,NTEIG
      DOUBLE PRECISION BIGNUM,EIGL,EIGR,ORIGIN,TRUSTL,TRUSTR
      LOGICAL          GNRZD,SLICE
C
      DOUBLE PRECISION EIG(LEIG,2),WORK(*),X(LNI,LEIG)
C
C==== local variables ==================================================
C
      INTEGER          NSLXI,NSRXI
...
C
      IF      ( NTEIG .EQ. 0     ) THEN
              CALL DSETLRM (11,LRWRN)
      ELSE IF ( NTEIG .LT. NREIG ) THEN
              CALL DSETLRM (13,LRWRN)
      END IF
C
C.... reorder the eigenpairs ...........................................
C 
      CALL DEIGSRT (LEIG,LNI,NTEIG,EIG,X)
C
C.... identify the interesting eigenpairs ..............................
C
      IF ( GNRZD ) THEN
         CALL DCOPY  (NTEIG,EIG,1,WORK,1)
         CALL DEIGIDX (INDXP,INDXS,NNSPNT,NPEIG,NREIG,NTEIG,
...
C
         NSLXI = DNEIGAB(NTEIG,TRUSTL,ORIGIN,EIG)
         NSRXI = DNEIGAB(NTEIG,ORIGIN,TRUSTR,EIG)
C
         IF      ( (NSLXI.EQ.0) .AND. (NSRXI.EQ.0) ) THEN 
                 CALL DSETLRM (12,LRWRN)
         ELSE IF ( (NSLXI+NSRXI).LT.NPEIG ) THEN
                 CALL DSETLRM (14,LRWRN)
         END IF
C
      END IF
C 
