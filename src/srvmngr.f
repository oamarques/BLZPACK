      SUBROUTINE SRVMNGR (JL,JT,NVB,LEIG,LNI,NI,NQMAX,NXMAX,FHNDL,LCOMM,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    SRVMNGR computes Ritz vectors                                    *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    SLZIOOP,SRVCOMP,SSETLRM,SSETTO0                                  *
C*                                                                     *
C*  - BLAS kernel:                                                     *
C*                                                                     *
C*    SCOPY                                                            *
...
C
      INTEGER          FHNDL(*)
      REAL             BASIS(*),BETA(NVB,NVB),EIG(LEIG,2),Q(LNI,NVB),
...
C
     ...
     &          (NEWR0.EQ.NVB) ) THEN
C
            CALL SSETTO0 (LNI*MAX(NEWR0,NRITZ),X(1,NCEIG+1),1)
C
            NQ = MIN(JL,NQMAX)*NVB
            NX = MAX(NRITZ,NEWR0)
            LBLAS = MIN(3,NX)
...
C
            IF ( NQ .GT. 0 ) CALL SRVCOMP (JT,LBLAS,LDB,LNI,NI,NQ,NX,
...
C
            IF ( JL .GT. NQMAX) THEN
               ...
               K = 1 + NI*NVB*NQMAX
               DO 10 I = 1,JL-NQMAX
                  CALL SLZIOOP (FHNDL,LCOMM,LRERR,I,NI*NVB,
     &                          BASIS(K),'Q ','GET')
                  IF ( LRERR .NE. 0 ) RETURN
                  CALL SRVCOMP (JT,LBLAS,NI,LNI,NI,NVB,NX,BASIS(K),
...
C*OAM          This needs to be tested
               CALL SRVCOMP (JT,LBLAS,LNI,LNI,NI,NVB,NX,
     &                       Q,WORK,X(1,NCEIG+1))
            END IF
C
...
C
            IF ( NXMAX.EQ.0 .AND. NRITZ.GT.0 ) THEN
               CALL SLZIOOP (FHNDL,LCOMM,LRERR,NTEIG,NI,
     &                      X(1,NCEIG+1),'X ','PUT')
               IF ( LRERR .NE. 0 ) RETURN
            END IF
C
