      SUBROUTINE SLZMMRY (JLMAX,JLSET,JTMAX,NVB,NVSET,LTAU,NI,N,LCOMM,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    SLZMMRY checks the workspace available                           *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C==== arguments ========================================================
C
      INTEGER   IBUSY,JLMAX,JLSET,JTMAX,LCOMM,LISTOR,LRERR,
     &          LRSTOR,LTAU,N,NBXMAX,NI,NPE,NQMAX,NRUNMX,
     &          NSIMAX,NVB,NVSET,RBUSY
      LOGICAL   GNRZD
C
C==== local variables ==================================================
C
...
C
C     the generalized problem requires (Q) and (B)*(Q) so FACTOR = 2 
C     the standard problem requires only (Q) so FACTOR = 1 
C
      IF ( GNRZD ) THEN
         FACTOR = 2
      ELSE
...
C.... minimum workspace ................................................
C
      CALL IPIRED ('MIN',1,LISTOR,TEMP,LCOMM,INFO)
      IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
      IF ( TEMP .LT. 0 ) LISTOR = -1
      CALL IPIRED ('MIN',1,LRSTOR,TEMP,LCOMM,INFO)
      IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
...
C
      TEMP = JLMAX
      CALL IPIRED ('MIN',1,TEMP,JLMAX,LCOMM,INFO)
      IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
      TEMP = JTMAX
      CALL IPIRED ('MIN',1,TEMP,JTMAX,LCOMM,INFO)
      IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
C
      MWORK = RBUSY + 15
...
C.... cross check error ................................................
C
      CALL IPIRED ('MAX',1,LRERR,TEMP,LCOMM,INFO)
      IF ( TEMP .NE. 0 ) CALL SSETLRM (23,LRERR)
      IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
C
C.... find the minima ..................................................
C
      TEMP = NVB
      CALL IPIRED ('MIN',1,TEMP,NVB,LCOMM,INFO)
      IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
C
      TEMP = NQMAX
      CALL IPIRED ('MIN',1,TEMP,NQMAX,LCOMM,INFO)
      IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
C
      TEMP = NBXMAX
      CALL IPIRED ('MIN',1,TEMP,NBXMAX,LCOMM,INFO)
      IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
C
   30 CONTINUE
C
