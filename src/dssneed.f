      SUBROUTINE DSSNEED (JT,JTMIN,NVB,LCOMM,NPE,LRMDE,NEWSIG,NSLS,NSRS,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DSSNEED checks for the need of a new origin translation          *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C
     ...
     &                 NSLS,NSRS,NSRLS,NSRRS,NVB
      DOUBLE PRECISION BIGNUM,EPS,GRNRM,THETAL,THETAR,THRES
C
      DOUBLE PRECISION TIME(*),RNORM(JT),THETA(JT)
C
C==== local variables ==================================================
C
      INTEGER          I,INDEX,INFO,J,NRNRM,NTSOL
      DOUBLE PRECISION C1,C2,W1,W2,W3,RSNRM,TBASIS,TEIGTB,TFACTR,
     &                 THTMAX,TDPURGE,TTOTAL,WEIGHT
C
      DOUBLE PRECISION T(5),TSUM(5)
C
C==== subprograms ======================================================
C
...
C
      ...
      T(5) = TBASIS
C
      CALL DPIRED ('SUM',5,T,TSUM,LCOMM,INFO)
C
      TFACTR = TSUM(1)/NPE
      TDPURGE = TSUM(2)/NPE
      TEIGTB = TSUM(3)/NPE
...
C
              W2 = WMIN
C
      ELSE IF ( TTOTAL .LT. TDPURGE*2 ) THEN
C
C             orthogonalization is expensive: take the maximum weight
C
              W2 = WMAX
C
      ELSE IF ( TTOTAL .GE. TDPURGE*5 ) THEN
C
...
C
      ELSE
C
C             linear model: W2 = f(TDPURGE/TTOTAL-0.2) with
C             W2(0.2) = WMIN and W2(0.5) = WMAX
C
              C1 = WMIN
              C2 = (WMAX-WMIN)/0.30D0
              W2 = C1 + C2*(TDPURGE/TTOTAL-0.20D0)
C
      END IF
C
...
C
     ...
     &          JTMIN*2.LT.JT .AND. WEIGHT.GT.ONE ) THEN
                NEWSIG = 2
      ELSE IF ( NTSOL.GT.0 .AND. DTBILLC(JT,EPS,THETA) ) THEN
                NEWSIG = 3
