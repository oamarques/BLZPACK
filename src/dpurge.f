      SUBROUTINE DPURGE (IDXETA,IDXTAU,JL,JLMAX,JT,NVB,LTAU,LNI,NI,N,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DPURGE performs reorthogonalizations                             *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    DPORTH,DREORTH,DRFACTR,DSORTH                                    *
C*                                                                     *
C*  - BLAS kernel:                                                     *
C*                                                                     *
C*    IDAMAX                                                           *
...
C==== arguments ========================================================
C
      INTEGER          IDXETA,IDXTAU,JL,JLMAX,JT,LCOMM,LNI,LRERR,
     &                 LRWRN,LTAU,N,NBXMAX,NI,NPORTH,NQMAX,NSORTH,
     &                 NTEIG,NULLDQ,NULLDR,NVB,NXMAX
      DOUBLE PRECISION ABSETA,ABSTAU,EPS,EPS1,REPS
      LOGICAL          GNRZD
C
      INTEGER          FHNDL(*),LBLAS(*)
      DOUBLE PRECISION ANORM(JLMAX,2),BASIS(*),BETAQ(*),BETAR(*),
     &                 BNORM(JLMAX,2),BQ(*),BR(*),BX(*),ETA(JLMAX,2),
     &                 Q(*),R(*),TAU(LTAU,2),WORK(*),X(*)
C
C==== local variables ==================================================
C
...
C
            SORTON = .TRUE.
C
            CALL DSORTH (LBLAS(4),LCOMM,LRERR,LNI,NI,NSORTH,NTEIG,NVB,
...
C
            PORTON = .TRUE.
C
            CALL DPORTH (JL,NVB,NI,LBLAS,LCOMM,LRERR,NPORTH,NQMAX,
...
C
              CALL DRFACTR (LBLAS(4),LCOMM,LRERR,LRWRN,NI,NVB,
...
C
      IF ( PORTON .OR. SORTON .OR. NVB.GT.1 .OR. N.GT.JT ) THEN
C
         CALL DREORTH (LBLAS,LCOMM,LRERR,LRWRN,NI,NVB,NULLDR,BQ,BR,Q,R,
