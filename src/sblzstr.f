      SUBROUTINE SBLZSTR (ISTOR,RSTOR,SIGMA,LFLAG,NVOPU,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    SBLZSTR prepares to start a Lanczos run                          *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    SLZCOPY,SLZPRT1,SRANDNR,SRVMNGR,SRVRSTR,SSETTO0,                 *
C*    SSITIME,SSORTH0,SSTARTR,SSTARTX                                 *
C*                                                                     *
...
C
              IF ( LFLAG .EQ. 4 ) THEN
                 ...
                 CALL SSTARTR (ISTOR(LCOMM),ISTOR(LRERR),ISTOR(LNI)  ,
     &                         ISTOR(NI)   ,NV          ,RSTOR(R)    ,
     &                         V           )
...
C............ initialize (R) ...........................................
C
              CALL SSETTO0 (ISTOR(NI)*ISTOR(NVB),RSTOR(R),1)
C
C............ choose Ritz vectors for restarting .......................
C
              CALL SRVRSTR (ISTOR(JT)    ,NV           ,ISTOR(NVB)   ,
...
C
      IF ( ISTOR(NRUN).EQ.0 .AND. ISTOR(NTEIG).NE.0 ) THEN
         CALL SSTARTX (ISTOR(LCOMM),ISTOR(LRERR),ISTOR(LNI)  ,
...
C
      CALL SLZPRT1 (ISTOR(LFILE) ,ISTOR(LPRNT) ,ISTOR(LRMDE) ,
     &              ISTOR(NRUN)  )
C
C.... if there are not enough Ritz vectors use random entries ..........
C
      CALL SRANDNR (ISTOR(LCOMM) ,ISTOR(LRERR) ,ISTOR(NI)    ,
...
C
      CALL SSORTH0 (LBLAS4       ,ISTOR(LCOMM) ,ISTOR(LRERR) ,
...
C
      IF ( GNRZD ) THEN
         CALL SLZCOPY (ISTOR(NI) ,ISTOR(LNI),ISTOR(NI),
     &                 ISTOR(NVB),RSTOR(R)  ,U        )
         AGAIN = .FALSE.
      ELSE
         AGAIN = .TRUE.
