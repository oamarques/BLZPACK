      SUBROUTINE DLZCHEK (JT,JTMAX,JTMIN,NVB,N,LCOMM,NPE,LRMDE,LRWRN,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DLZCHEK checks the status of the run                             *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C
      DOUBLE PRECISION THETA(JT,2),TIME(*)
C
C==== subprogram =======================================================
C
      LOGICAL          DTBILLC
...
C
              CALL DSSNEED (JT,JTMIN,NVB,LCOMM,NPE,LRMDE,NEWSIG,
     &                      NSLS,NSRS,NSRLS,NSRRS,BIGNUM,EPS,
     &                      GRNRM,THETA(1,2),THETA(1,1),
     &                      THETAL,THETAR,THRES,TIME)
C
              IF ( NEWSIG.EQ.3 .AND. LRMDE.GT.6 ) CALL DSETLRM (2,LRWRN)
              EIGON = NEWSIG.GT.1
C
      ELSE IF ( GNRZD ) THEN
C
...
C
              IF ( DTBILLC(JT,EPS,THETA) ) THEN
C
C............... operator is assumed to be ill conditioned .............
C
                 CALL DSETLRM (2,LRWRN)
                 EIGON = .TRUE.
C
              ELSE
C
...
C
      IF ( JT.GE.N .AND. (NSLS+NSRS).EQ.0 ) CALL DSETLRM (1,LRWRN)
C
      RETURN
C
C**** end of DLZCHEK ***************************************************
