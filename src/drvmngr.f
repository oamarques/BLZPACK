      SUBROUTINE DRVMNGR (JL,JT,NVB,LEIG,LNI,NI,NQMAX,NXMAX,FHNDL,LCOMM,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DRVMNGR computes Ritz vectors                                    *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*    NI     (sii) : dimension of the vectors in (Q) and (X)           *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    DLZIOOP,DRVCOMP,DSETLRM,DSETTO0                                  *
C*                                                                     *
C*  - BLAS kernel:                                                     *
C*                                                                     *
C*    DCOPY                                                            *
...
C==== arguments ========================================================
C
      INTEGER          JL,JT,LCOMM,LEIG,LNI,LRERR,LRWRN,NI,
     &                 NQMAX,NTEIG,NVB,NXMAX
      DOUBLE PRECISION SIGMA,THETA0
      LOGICAL          GNRZD,PURFY
C
      INTEGER          FHNDL(*)
      DOUBLE PRECISION BASIS(*),BETA(NVB,NVB),EIG(LEIG,2),Q(LNI,NVB),
     &                 RITZ(JT,2),S(JT,*),WORK(JT,*),X(LNI,*)
C
C==== local variables ==================================================
C
...
C
     ...
     &          (NEWR0.EQ.NVB) ) THEN
C
            CALL DSETTO0 (LNI*MAX(NEWR0,NRITZ),X(1,NCEIG+1),1)
C
            NQ = MIN(JL,NQMAX)*NVB
            NX = MAX(NRITZ,NEWR0)
            LBLAS = MIN(3,NX)
...
C
            IF ( NQ .GT. 0 ) CALL DRVCOMP (JT,LBLAS,LDB,LNI,NI,NQ,NX,
...
C
            IF ( JL .GT. NQMAX) THEN
               ...
               K = 1 + NI*NVB*NQMAX
               DO 10 I = 1,JL-NQMAX
                  CALL DLZIOOP (FHNDL,LCOMM,LRERR,I,NI*NVB,
     &                          BASIS(K),'Q ','GET')
                  IF ( LRERR .NE. 0 ) RETURN
                  CALL DRVCOMP (JT,LBLAS,NI,LNI,NI,NVB,NX,BASIS(K),
...
C
   ...
   30             CONTINUE
   40          CONTINUE
C*OAM          This needs to be tested
               CALL DRVCOMP (JT,LBLAS,LNI,LNI,NI,NVB,NX,
     &                       Q,WORK,X(1,NCEIG+1))
            END IF
C
...
C
            IF ( NXMAX.EQ.0 .AND. NRITZ.GT.0 ) THEN
               CALL DLZIOOP (FHNDL,LCOMM,LRERR,NTEIG,NI,
     &                      X(1,NCEIG+1),'X ','PUT')
               IF ( LRERR .NE. 0 ) RETURN
            END IF
C
