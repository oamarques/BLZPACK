      SUBROUTINE SSORTH (LBLAS,LCOMM,LRERR,LNI,NI,NSORTH,NTEIG,NVB,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    SSORTH performs a selective orthogonalization                    *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C
      INTEGER          FHNDL(*)
      REAL             BR(NI,NVB),BX(NI,*),R(NI,NVB),TAUQ(*),
...
C
         IF ( GNRZD ) THEN
            ...
            ELSE
               IBX = NBXMAX + 1
               CALL SLZIOOP (FHNDL,LCOMM,LRERR,I-NBXMAX,
     &                       NI,BX(1,IBX),'BX','GET')
...
C
         ...
         ELSE
            IX = 1
            CALL SLZIOOP (FHNDL,LCOMM,LRERR,I,NI,X(1,IX),'X ','GET')
            IF ( LRERR .NE. 0 ) RETURN
         END IF
C
...
C
         IF ( GNRZD ) THEN 
            CALL SQTBR   (NI,BX(1,IBX),NI,1,R,NI,NVB,ZETA,
     &                    ZMAX,WORK,LBLAS,LCOMM,LRERR)
            CALL SRQALPH (NI,BR,NI,NVB,BX(1,IBX),NI,1,ZETA,LBLAS)
            CALL SRQALPH (NI,R ,NI,NVB,X(1,IX)  ,NI,1,ZETA,LBLAS)
         ELSE
            CALL SQTBR   (NI, X(1,IX) ,NI,1,R,NI,NVB,ZETA,
     &                    ZMAX,WORK,LBLAS,LCOMM,LRERR)
            CALL SRQALPH (NI,R ,NI,NVB,X(1,IX)  ,NI,1,ZETA,LBLAS)
         END IF
C
