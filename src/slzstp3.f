      SUBROUTINE SLZSTP3 (IR,JL,JLMAX,JT,NVB,LEIG,LTAU,LNI,NI,N,FHNDL,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    SLZSTP3 factors (R) and controls orthogonality level             *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    SPURGE,SSETLRM,SSETTO0,SSITIME,SUPETA,SUPTAU                     *
C*                                                                     *
...
C
     ...
     &                 NSORTH,NTEIG,NULLDQ,NULLDR,NVB,NXMAX
      REAL             ABSETA,ABSTAU,ENDL,ENDR,EPS,EPS1,
     &                 REPS,SIGMA,THETA0
      LOGICAL          GNRZD
C
      INTEGER          FHNDL(*),IR(4),LBLAS(*)
      REAL             ALPHA(NVB,NVB),ANORM(JLMAX,2),BASIS(*),
...
C
         IF ( NULLDR .GT. 0 ) CALL SSETLRM (3,LRWRN)
         CALL SSETTO0 (JLMAX*2,ETA,1) 
         ABSETA = ZERO
         ABSTAU = ZERO
         IDXETA = 0
         IDXTAU = 0
...
C
         CALL SUPETA (JL,ANORM(1,2),BNORM(1,2),BNORM(JLP1,1),
     &                EPS1,ETA(1,1),ETA(1,2))
C
         CALL SUPTAU (LEIG,NTEIG,NVB,ALPHA,BNORM(JLP1,2),
...
C
         CALL SPURGE (IDXETA,IDXTAU,JL,JLMAX,JT,NVB,LTAU,LNI,NI,N,FHNDL,
...
C
         TSPURGE = SSITIME(TIME0)
C
         TIME(5) = TIME(5) + TSPURGE
         TIME(9) = TIME(9) + TSPURGE
C
      END IF
C
      RETURN 
