      SUBROUTINE DSSORGN (LRMDE,LRWRN,NSIMAX,NESIKL,NESIKR,NEWSIG,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DSSORGN sets the origin translation                              *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C
     ...
     &                 SIGMA,TRUSTL,TRUSTR
C
      DOUBLE PRECISION RSINT(6,NSIMAX)
C
C==== local variables ==================================================
C
      INTEGER          I
...
C
   20 CONTINUE
C
C.... decide what to do upon restarting ................................
C
      IF      ( DSSRSTR(LRMDE,NESIKL,NESIKR,NEWSIG,NSLS,NSRS,
...
C
              LOWER  = RSINT(1,INDSI)
              UPPER  = RSINT(6,INDSI)
              EXTNDL = LOWER.LT.ORIGIN .AND. SIGMA.EQ.UPPER .AND. 
     &                 SFARL.NE.EIGL
              EXTNDR = UPPER.GT.ORIGIN .AND. SIGMA.EQ.LOWER .AND.
     &                 SFARR.NE.EIGR
C
...
C
                              SIGMA = RSINT(4,INDSI)
C
                              IF (SIGMA.EQ.UPPER) SIGMA = SIGMA - RADIUS
C
...
C
                              SIGMA = RSINT(3,INDSI)
C
                              IF (SIGMA.EQ.LOWER) SIGMA = SIGMA + RADIUS
C
                      END IF
C
...
C
                      CALL DSSSPLT (INDSI,SIGMA,RSINT)
C
                      LRMDE = 4
C
              END IF
