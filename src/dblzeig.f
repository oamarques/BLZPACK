      SUBROUTINE DBLZEIG (ISTOR,RSTOR,LFLAG,SIGMA,NNEIG,EIG,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DBLZEIG deals with the converged eigenvalues and eigenvectors    *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    DRVMNGR,DSETLRM,DSETSSL,DSITIME,DSSBEXT,                         *
C*    DSSCHEK,DSSORGN,DSSSPEC,DSSTRSF,DTBEIGP                          *
C*                                                                     *
...
C
C.... prepare to find more eigenvalues/eigenvectors if needed ..........
C
...
C
      ELSE IF ( SLICE ) THEN
C
C............ store subinterval information ............................
C
              NCEIG = NSLS + NSRS
C
              CALL DSETSSL (ISTOR(JL)    ,ISTOR(JT)    ,NNEIG        ,
     &                      NCEIG        ,RSTOR(ENDL)  ,RSTOR(ENDR)  ,
     &                      SIGMA        ,ISTOR(NSLOG) ,RSTOR(SSLOG) ,
     &                      RSTOR(TIME)  )
C
C............ compute new shift candidates .............................
C
              CALL DSSSPEC (ISTOR(JT)    ,ISTOR(LFILE) ,ISTOR(LPRNT) ,
     ...
     &                      SIGMAL       ,SIGMAR       )
C
C............ extend interval boundaries ...............................
C
              CALL DSSBEXT (ISTOR(NSINT) ,RITZL        ,RITZR        ,
     &                      SIGMA        ,SIGMAL       ,SIGMAR       ,
     &                      RSTOR(RSINT) )
C
C............ check the subintervals ...................................
C
              CALL DSSCHEK (ISTOR(LRERR) ,ISTOR(LRMDE) ,NESIKL       ,
...
C
C............ set origin translation ...................................
C
              CALL DSSORGN (ISTOR(LRMDE) ,ISTOR(LRWRN) ,ISTOR(NSIMAX),
