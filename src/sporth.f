      SUBROUTINE SPORTH (JL,NVB,NI,LBLAS,LCOMM,LRERR,NPORTH,NQMAX,BASIS,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    SPORTH performs a partial reorthogonalization                    *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C
      INTEGER          FHNDL(*),LBLAS(*)
      REAL             BASIS(*),BQ(NI,NVB),BR(NI,NVB),ETAQ(*),ETAR(*),
...
C
         IF ( J .GT. NQMAX ) THEN
            ...
            IBQ = IBQOUT
            LDB = NI
            CALL SLZIOOP (FHNDL,LCOMM,LRERR,J-NQMAX,NI*NVB,BASIS(IQ),
     &                    'Q ','GET')
            IF ( LRERR .NE. 0 ) RETURN
            IF ( GNRZD ) THEN
               CALL SLZIOOP (FHNDL,LCOMM,LRERR,J-NQMAX,NI*NVB,
     &                       BASIS(IBQ),'BQ','GET')
               IF ( LRERR .NE. 0 ) RETURN
            END IF
         END IF
...
C
         CALL SQTBR (NI,BASIS(IBQ),LDB,NVB,Q,NI,NVB,ZETA(IZETA1),
     &              DUMMY,WORK,LBLAS(2),LCOMM,LRERR)
         CALL SQTBR (NI,BASIS(IBQ),LDB,NVB,R,NI,NVB,ZETA(IZETA2),
     &              DUMMY,WORK,LBLAS(2),LCOMM,LRERR)
C
C....... orthogonalization of (Q) and (R) against old (Q) ..............
C
         CALL SRQALPH (NI,Q,NI,NVB,BASIS(IQ),LDB,NVB,
     &                 ZETA(IZETA1),LBLAS(3))
         CALL SRQALPH (NI,R,NI,NVB,BASIS(IQ),LDB,NVB,
     &                 ZETA(IZETA2),LBLAS(3))
C
C....... orthogonalization of (BQ) and (BR) against old (B)*(R) ........
C
         IF ( GNRZD ) THEN
            CALL SRQALPH (NI,BQ,NI,NVB,BASIS(IBQ),LDB,NVB,
     &                    ZETA(IZETA1),LBLAS(3))
            CALL SRQALPH (NI,BR,NI,NVB,BASIS(IBQ),LDB,NVB,
     &                    ZETA(IZETA2),LBLAS(3))
         END IF
C
