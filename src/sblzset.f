      SUBROUTINE SBLZSET (IPSET,RPSET,ISTOR,RSTOR,SIGMA,
     &                    LFLAG,AGAIN,STRON)
C
C***********************************************************************
C*                                                                     *
C*  - Purpose:                                                         *
C*                                                                     *
C*    SBLZSET initializes basic variables                              *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    BLASDEF,SDATCHK,SDATSET,SLZERRS,SLZPNTR,SSETDFT,SSITIME          *
C*                                                                     *
C***********************************************************************
C
C==== parameters =======================================================
...
C.... set defaults .....................................................
C
      CALL SSETDFT (ITEMP(JTMIN) ,ITEMP(NVB)   ,ITEMP(NRUNMX),
     &              ITEMP(NSIMAX),ITEMP(NXMAX) ,ITEMP(FHNDL) ,
     &              RTEMP(BIGNUM),RTEMP(EPS)   )
C
C.... check input data .................................................
C
      CALL SDATCHK (IPSET   ,RPSET      ,ITEMP(LRERR),
...
C
      ...
      RBUSY = RUSED
C
      CALL SDATSET (IPSET        ,RPSET        ,ITEMP(JLMAX) ,
...
C
              CALL SLZERRS (ITEMP(LFILE),ITEMP(LRERR))
              LFLAG = -ITEMP(LRERR)  
              RETURN
C
...
C
      OFFSET = RUSED + 1
C
C     Note that SLZPNTR defines 18 addresses from ISTOR(ITIME)
C
      CALL SLZPNTR (OFFSET       ,ISTOR(ITIME) ,ISTOR(JLMAX) ,
