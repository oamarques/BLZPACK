      SUBROUTINE DLZSTP4 (IR,JL,JTMAX,NVB,NI,FHNDL,LCOMM,LRERR,NQMAX,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DLZSTP4 stores (ALPHA), (BETA) and Lanczos vectors               *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    DLZCOPY,DLZIOOP,DTBALPH,DTBBETA                                  *
C*                                                                     *
C*  - BLAS kernel:                                                     *
C*                                                                     *
C*    DCOPY                                                            *
...
C==== arguments ========================================================
C
      INTEGER          JL,JTMAX,LCOMM,LRERR,NI,NQMAX,NVB
      LOGICAL          GNRZD
C
      INTEGER          FHNDL(*),IR(4)
...
C
      IF ( JL .GT. 1 ) CALL DTBBETA (JL-1,JTMAX,NVB,BETAQ,TB)
                       CALL DTBALPH (JL  ,JTMAX,NVB,ALPHA,TB)
C
...
C
         IF (GNRZD) THEN
            IQ = 1 + NI*NVB*(JL-1)*2
            CALL DLZCOPY (NI,NI*2,NI,NVB,R(IR(2)),BASIS(IQ+NI))
            CALL DLZCOPY (NI,NI*2,NI,NVB,R(IR(4)),BASIS(IQ   ))
         ELSE
            IQ = 1 + NI*NVB*(JL-1)
            CALL DLZCOPY (NI,NI  ,NI,NVB,R(IR(4)),BASIS(IQ   ))
         END IF
C
      ELSE 
C
...
C
         IF (GNRZD) THEN
            CALL DLZIOOP (FHNDL,LCOMM,LRERR,JL-NQMAX,NI*NVB,
     &                    R(IR(2)),'BQ','PUT')
