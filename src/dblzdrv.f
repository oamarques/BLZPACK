...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DBLZDRV is the user interface for a set of subprograms intended  *
...
C*  - Arguments:                                                       *
C*                                                                     *
C*    ISTOR  : integer array of dimension 20+LISTOR (defined below).   *
C*             On the first call to DBLZDRV, the first 15 entries of   *
...
C*                               LNI >= NI                             *
C*             ISTOR( 3)=NREIG,  number of required eigenpairs         *
C*             ISTOR( 4)=LEIG,   maximum number of eigenpairs,         *
...
C*                                                                     *
C*    RSTOR  : real array of dimension 10+LRSTOR (define below). On    *
C*             the first call to DBLZDRV, the first 4 entries of       *
C*             RSTOR must contain:                                     *
C*                                                                     *
...
C*                                                                     *
C*             < 0, early or abnormal termination                      *
C*             = 0, initialization or normal termination               *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    DBLZEIG,DBLZEND,DBLZEXT,DBLZFCT,DBLZKLL,                         *
C*    DBLZRBX,DBLZSET,DBLZSTP,DBLZSTR                                  *
C*                                                                     *
C***********************************************************************
C*                                                                     *
C*    LISTOR .ge. 150 + k1 x 12                                        *
C*                                                                     *
...
C*    where                                                            *
C*                                                                     *
C*    k1 = NVBIN x NLSIN if NVBIN x NLSIN > 0,                         *
C*         min(N,180) otherwise                                        *
C*                                                                     *
C*    k2 = NVBIN if NVBIN > 0,                                         *
C*         3 otherwise                                                 *
C*                                                                     *
C*    k3 = 500 + k1 x ( 13 + k1 x 2 + k2 + max(18,k2+2) ) +            *
C*         k2 x k2 x 3 + k4 x 2                                        *
C*                                                                     *
C*    k4 = min(LEIG,N)                                                 *
C*                                                                     *
C*    Temporary files may be created in sequential mode depending on   *
C*    the value of LRSTOR, potentially impacting the computational     *
C*    performance of BLZPACK. These user is advised to set LRSTOR      *
C*    as if running in parallel mode.                                  *
C*                                                                     *
C***********************************************************************
C
C==== parameters =======================================================
...
C
              CALL DBLZEIG (ISTOR(IINIT),RSTOR(RINIT),LFLAG,SIGMA,NNEIG,
...
C............ finish ...................................................
C
              CALL DBLZEND (ISTOR(IINIT),RSTOR(RINIT),LFLAG,EIG,X,ABORT)
C
...
C
      ELSE IF ( KLLON ) THEN
C
C............ termination enforced .....................................
C
              CALL DBLZKLL (ISTOR(IINIT),RSTOR(RINIT),SIGMA,EIG,X,U)
C
              KLLON = .FALSE.
              AGAIN = .TRUE.
              ENDON = .TRUE.
...
C............ compute (B)*(X) ..........................................
C
              CALL DBLZRBX (ISTOR(IINIT),RSTOR(RINIT),LFLAG,NVOPU,
...
C
              CALL DBLZSTP (ISTOR(IINIT),RSTOR(RINIT),SIGMA,NNEIG,LFLAG,
...
C
              CALL DBLZSTR (ISTOR(IINIT),RSTOR(RINIT),SIGMA,LFLAG,
...
C
C............ DBLZDRV is called for the first time .....................
C
              CALL DBLZSET (ISTOR,RSTOR,ISTOR(IINIT),RSTOR(RINIT),
...
C
              CALL DBLZFCT (ISTOR(IINIT),RSTOR(RINIT),SIGMA,NNEIG,
...
C
      CALL DBLZEXT (ISTOR,RSTOR,LFLAG)
C
      RETURN 
C
C**** end of DBLZDRV ***************************************************
