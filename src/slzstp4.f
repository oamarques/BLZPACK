      SUBROUTINE SLZSTP4 (IR,JL,JTMAX,NVB,NI,FHNDL,LCOMM,LRERR,NQMAX,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    SLZSTP4 stores (ALPHA), (BETA) and Lanczos vectors               *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    SLZCOPY,SLZIOOP,STBALPH,STBBETA                                  *
C*                                                                     *
C*  - BLAS kernel:                                                     *
C*                                                                     *
C*    SCOPY                                                            *
...
C
      IF ( JL .GT. 1 ) CALL STBBETA (JL-1,JTMAX,NVB,BETAQ,TB)
                       CALL STBALPH (JL  ,JTMAX,NVB,ALPHA,TB)
C
...
C
         IF (GNRZD) THEN
            IQ = 1 + NI*NVB*(JL-1)*2
            CALL SLZCOPY (NI,NI*2,NI,NVB,R(IR(2)),BASIS(IQ+NI))
            CALL SLZCOPY (NI,NI*2,NI,NVB,R(IR(4)),BASIS(IQ   ))
         ELSE
            IQ = 1 + NI*NVB*(JL-1)
            CALL SLZCOPY (NI,NI  ,NI,NVB,R(IR(4)),BASIS(IQ   ))
         END IF
C
      ELSE 
C
...
C
         IF (GNRZD) THEN
            CALL SLZIOOP (FHNDL,LCOMM,LRERR,JL-NQMAX,NI*NVB,
     &                    R(IR(2)),'BQ','PUT')
