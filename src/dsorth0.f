      SUBROUTINE DSORTH0 (LBLAS,LCOMM,LRERR,LEIG,LNI,NI,NSORTH,NTEIG,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DSORTH0 purges the starting vectors and sets TAU                 *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C==== arguments ========================================================
C
      INTEGER          LBLAS,LCOMM,LEIG,LNI,LRERR,NI,
     &                 NSORTH,NTEIG,NVB,NXMAX,NBXMAX
      DOUBLE PRECISION ENDL,ENDR,EPS1
      LOGICAL          GNRZD
C
      INTEGER          FHNDL(*)
      DOUBLE PRECISION BX(NI,*),EIG(LEIG,2),R(NI,NVB),TAUQ(*),
     &                 TAUR(*),X(LNI,*),WORK(*),ZETA(*)
C
C==== local variables ==================================================
C
      INTEGER          I,IBX,IX
...
C
              ...
              ELSE
                 IX = 1
                 CALL DLZIOOP (FHNDL,LCOMM,LRERR,I,NI,X(1,IX),
     &                         'X ','GET')
                 IF ( LRERR .NE. 0 ) RETURN
              END IF
C
