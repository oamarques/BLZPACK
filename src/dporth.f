      SUBROUTINE DPORTH (JL,NVB,NI,LBLAS,LCOMM,LRERR,NPORTH,NQMAX,BASIS,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DPORTH performs a partial reorthogonalization                    *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    DLZIOOP,DQTBR,DRQALPH                                            *
C*                                                                     *
C***********************************************************************
C
C==== arguments ========================================================
C
      INTEGER          JL,LCOMM,LRERR,NI,NPORTH,NQMAX,NVB
      DOUBLE PRECISION EPS1
      LOGICAL          GNRZD
C
      INTEGER          FHNDL(*),LBLAS(*)
      DOUBLE PRECISION BASIS(*),BQ(NI,NVB),BR(NI,NVB),ETAQ(*),ETAR(*),
     &                 Q(NI,NVB),R(NI,NVB),ZETA(*),WORK(*)
C
C==== local variables ==================================================
C
...
C
         IF ( J .GT. NQMAX ) THEN
            ...
            IBQ = IBQOUT
            LDB = NI
            CALL DLZIOOP (FHNDL,LCOMM,LRERR,J-NQMAX,NI*NVB,BASIS(IQ),
     &                    'Q ','GET')
            IF ( LRERR .NE. 0 ) RETURN
            IF ( GNRZD ) THEN
               CALL DLZIOOP (FHNDL,LCOMM,LRERR,J-NQMAX,NI*NVB,
     &                       BASIS(IBQ),'BQ','GET')
               IF ( LRERR .NE. 0 ) RETURN
            END IF
         END IF
...
C
         CALL DQTBR (NI,BASIS(IBQ),LDB,NVB,Q,NI,NVB,ZETA(IZETA1),
     &              DUMMY,WORK,LBLAS(2),LCOMM,LRERR)
         CALL DQTBR (NI,BASIS(IBQ),LDB,NVB,R,NI,NVB,ZETA(IZETA2),
     &              DUMMY,WORK,LBLAS(2),LCOMM,LRERR)
C
C....... orthogonalization of (Q) and (R) against old (Q) ..............
C
         CALL DRQALPH (NI,Q,NI,NVB,BASIS(IQ),LDB,NVB,
     &                 ZETA(IZETA1),LBLAS(3))
         CALL DRQALPH (NI,R,NI,NVB,BASIS(IQ),LDB,NVB,
     &                 ZETA(IZETA2),LBLAS(3))
C
C....... orthogonalization of (BQ) and (BR) against old (B)*(R) ........
C
         IF ( GNRZD ) THEN
            CALL DRQALPH (NI,BQ,NI,NVB,BASIS(IBQ),LDB,NVB,
     &                    ZETA(IZETA1),LBLAS(3))
            CALL DRQALPH (NI,BR,NI,NVB,BASIS(IBQ),LDB,NVB,
     &                    ZETA(IZETA2),LBLAS(3))
         END IF
C
