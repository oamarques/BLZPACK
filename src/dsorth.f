      SUBROUTINE DSORTH (LBLAS,LCOMM,LRERR,LNI,NI,NSORTH,NTEIG,NVB,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DSORTH performs a selective orthogonalization                    *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C==== arguments ========================================================
C
      INTEGER          LBLAS,LCOMM,LNI,LRERR,NBXMAX,
     &                 NI,NSORTH,NTEIG,NVB,NXMAX
      DOUBLE PRECISION EPS1,REPS
      LOGICAL          GNRZD
C
      INTEGER          FHNDL(*)
      DOUBLE PRECISION BR(NI,NVB),BX(NI,*),R(NI,NVB),TAUQ(*),
     &                 TAUR(*),X(LNI,*),ZETA(*),WORK(*)
C
C==== local variables ==================================================
C
      INTEGER          I,IBX,IX
...
C
         IF ( GNRZD ) THEN
            ...
            ELSE
               IBX = NBXMAX + 1
               CALL DLZIOOP (FHNDL,LCOMM,LRERR,I-NBXMAX,
     &                       NI,BX(1,IBX),'BX','GET')
...
C
         ...
         ELSE
            IX = 1
            CALL DLZIOOP (FHNDL,LCOMM,LRERR,I,NI,X(1,IX),'X ','GET')
            IF ( LRERR .NE. 0 ) RETURN
         END IF
C
...
C
         IF ( GNRZD ) THEN 
            CALL DQTBR   (NI,BX(1,IBX),NI,1,R,NI,NVB,ZETA,
     &                    ZMAX,WORK,LBLAS,LCOMM,LRERR)
            CALL DRQALPH (NI,BR,NI,NVB,BX(1,IBX),NI,1,ZETA,LBLAS)
            CALL DRQALPH (NI,R ,NI,NVB,X(1,IX)  ,NI,1,ZETA,LBLAS)
         ELSE
            CALL DQTBR   (NI, X(1,IX) ,NI,1,R,NI,NVB,ZETA,
     &                    ZMAX,WORK,LBLAS,LCOMM,LRERR)
            CALL DRQALPH (NI,R ,NI,NVB,X(1,IX)  ,NI,1,ZETA,LBLAS)
         END IF
C
