      SUBROUTINE SRFACTR (LBLAS,LCOMM,LRERR,LRWRN,NI,NVB,NULLP,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    SRFACTR performs the factorization (R)=(Q)*(BETA)                *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    SMGSCHM,SNORM2A,SSETLRM,SSETTO0,SUPBETA                          *
C*                                                                     *
C***********************************************************************
C
C==== parameters =======================================================
...
C
      ...
      REAL             AMAXN,BMAXN,BMINN,EPS
      LOGICAL          GNRZD
C
      REAL             BETA(NVB,NVB),BR(NI,NVB),DELTA(NVB,NVB),
...
C
         CALL SMGSCHM (LBLAS,LCOMM,LRERR,NI,NVB,BR,R,
     &                 DELTA,WORK,GNRZD,NPIVT)
C
C....... updating: (BETA) := (DELTA)*(BETA) ............................
C
         CALL SUPBETA (NVB,BETA,DELTA)
C
...
C
 	    CALL SNORM2A (NVB,BETA,WORK,EPS,BMAXN,BMINN) 
C
...
C
                 DO 40 J = 1,NVB
                    IF ( BETA(J,J) .LE. EPS*HNDRD ) THEN
                       IF ( GNRZD ) CALL SSETTO0 (NI,BR(1,J),1)
	                            CALL SSETTO0 (NI, R(1,J),1)
                       NULLP = NULLP + 1
                       DO 30 K = 1,NVB
                          BETA(J,K)  = ZERO 
   30                  CONTINUE
