      SUBROUTINE SLZSTP1 (IR,JL,JLMAX,JT,NVB,LNI,NI,LBLAS,LCOMM,LRERR,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    SLZSTP1 computes (R)=(V)-(Q_{j})*(ALPHA_{j})                     *
C*                            -(Q_{j-1})*(BETA_{j})                    *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    SLZCOPY,SNORM2A,SQTBR,SRQALPH,SRQBETA,SSITIME                    *
C*                                                                     *
C***********************************************************************
C
C==== parameter ========================================================
...
C
      TIME0 = SSITIME(ZERO)
      TIME(1) = TIME(1) + SSITIME(TIME(11))
C
      CALL SLZCOPY (LNI,NI,NI,NVB,V,R(IR(1))) 
C
C.... orthogonalize (R) against the j-1 th (Q) .........................
C
      IF ( JL .GT. 1 ) CALL SRQBETA (R(IR(1)),R(IR(3)),BETAR,
     &                               LBLAS(1),NI,NVB)
C
C.... orthogonalize (R) against the j   th (Q) .........................
C
      CALL SQTBR   (NI,R(IR(2)),NI,NVB,R(IR(1)),NI,NVB,ALPHA,
     &              DUMMY,WORK,LBLAS(2),LCOMM,LRERR)
C
      CALL SRQALPH (NI,R(IR(1)),NI,NVB,R(IR(4)),NI,NVB,ALPHA,LBLAS(3))
C
      CALL SNORM2A (NVB,ALPHA,WORK,EPS,ANORM(JL,2),ANORM(JL,1))
C
C.... set for (BR)=op(B)*(R) if (A)*(x)-eig*(B)*(x)=(0) ................
C
      IF ( GNRZD ) THEN
         CALL SLZCOPY (NI,LNI,NI,NVB,R(IR(1)),U)
         NMOPB = NMOPB + NVB
      END IF
C
