      SUBROUTINE DSSBNDS (LRWRN,N,NNEIG,NDEIG,NFARL,NFARR,NNSPNT,NNTRTL,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    DSSBNDS initializes the computational subintervals               *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    DNEIGAB,DSETLRM,DSETTO0,DSITIME,DSSMOVB                          *
C*                                                                     *
...
C==== arguments ========================================================
C
      INTEGER          INDSI,LRWRN,N,NFARL,NFARR,NDEIG,NNEIG,NNSPNT,
     &                 NNTRTL,NNTRTR,NREIGL,NREIGR,NSIGMA,NSIMAX,
     &                 NSINT,NSRLS,NSRRS,NTEIG
      DOUBLE PRECISION BIGNUM,EIGL,EIGR,ENDL,ENDR,ORIGIN,SFARL,SFARR,
     &                 SIGMA,THETA0,THETAL,THETAR,TRUSTL,TRUSTR
C
      INTEGER          ISINT(2,*)
      DOUBLE PRECISION EIG(*),RSINT(6,*),TIME(*)
C
C==== local variables ==================================================
C
...
C
         ...
         SFARR  = SIGMA
C
         IF ( (NREIGL.EQ.0) .AND. (NREIGR.EQ.0) ) CALL DSETLRM (1,LRWRN)
C
         CALL DSETTO0 (NSIMAX*6,RSINT,1)
C
...
C.... update bounds ....................................................
C
      CALL DSSMOVB(-1,NSINT,INDSI+1,ISINT,RSINT)
C
      ISINT(2,INDSI  ) = NNEIG 
...
C
      IF ( SIGMA .LE. ORIGIN ) THEN 
         NFARL = NNEIG
         SFARL = SIGMA
         IF ( NREIGL.LE.(NNSPNT-NNEIG) .OR. SIGMA.LE.EIGL ) THEN
            ISINT(1,    1) = NNEIG 
            RSINT(1,    1) = SIGMA 
            RSINT(2,    1) = SIGMA 
            RSINT(3,    1) = SIGMA 
...
C
      IF ( SIGMA .GE. ORIGIN ) THEN 
         NFARR = NNEIG
         SFARR = SIGMA
         IF ( NREIGR.LE.(NNEIG-NNSPNT) .OR. SIGMA.GE.EIGR ) THEN
            ISINT(2,NSINT) = NNEIG 
            RSINT(6,NSINT) = SIGMA 
            RSINT(5,NSINT) = SIGMA 
            RSINT(4,NSINT) = SIGMA 
...
C
     ...
     &     SIGMA.LT.ORIGIN .AND. NNEIG.EQ.NNSPNT ) 
     &     CALL DSETLRM (1,LRWRN)
C
      IF ( SIGMA.GE.EIGL .AND. SIGMA.GE.EIGR .AND.
     &     SIGMA.GT.ORIGIN .AND. NNEIG.EQ.NNSPNT ) 
     &     CALL DSETLRM (1,LRWRN)
C
      RETURN 
C
