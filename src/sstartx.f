      SUBROUTINE SSTARTX (LCOMM,LRERR,LNI,NI,NX,X)
C
C***********************************************************************
C*                                                                     *
C*  - Purpose:                                                         *
C*                                                                     *
C*    SSTARTX checks the starting eigenvectors defined by the user     *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C....... check for NaNs ................................................
C
         IF ( SUMX .NE. SUMX ) CALL SSETLRM (26,LRERR)
         CALL IPIRED ('MAX',1,LRERR,GERR,LCOMM,INFO)
         IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
         IF ( GERR .NE. 0 ) CALL SSETLRM (26,LRERR)
C
C....... check for a zero X ............................................
C
         IF ( LRERR .EQ. 0 ) THEN
            CALL SPIRED ('MAX',1,SUMX,GSUM,LCOMM,INFO)
            IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
            IF ( GSUM .EQ. 0 ) CALL SSETLRM (26,LRERR)
         END IF
C
   10 CONTINUE
C
