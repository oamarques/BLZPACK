      SUBROUTINE SLZNRMX (NVB,LNI,NI,FHNDL,LCOMM,LRERR,LRWRN,NMOPB,NBX,
...
C*  - Purpose:                                                         *
C*                                                                     *
C*    SLZNRMX deals with the product (B)*(X)                           *
C*                                                                     *
C*  - Arguments:                                                       *
C*                                                                     *
...
C*  - Subprograms:                                                     *
C*                                                                     *
C*    SLZCOPY,SLZIOOP,SPIRED,SSETLRM,SSITIME                           *
C*                                                                     *
C*  - BLAS kernels:                                                    *
C*                                                                     *
C*    SDOT,SSCAL                                                       *
...
C
            ...
            CALL SPIRED ('SUM',1,TEMP,ZETA,LCOMM,INFO)
            IF ( INFO .NE. 0 ) CALL SSETLRM (32,LRERR)
C
            IF ( ZETA .LT. ZERO ) THEN
               CALL SSETLRM (15,LRWRN)
               RETURN
            ELSE
               ZETA = ONE/SQRT(ZETA)                    
            END IF
...
C
               IF ( NXMAX .EQ. 0 ) THEN
                  CALL SLZIOOP (FHNDL,LCOMM,LRERR,NBX,NI,
     &                          U(1,I),'X ','PUT')
                  IF ( LRERR .NE. 0 ) RETURN
               ELSE
                  CALL SLZCOPY (LNI,NI,NI,1,U(1,I),X(1,NBX))
               END IF
C
            END IF
C
            TIME(7) = TIME(7) + SSITIME(TIME0)
C
         END IF
C
...
C
         IF ( NBXMAX .LT. NBX ) THEN
            J = NBX - NBXMAX
            CALL SLZIOOP (FHNDL,LCOMM,LRERR,J,NI,V(1,I),'BX','PUT')
            IF ( LRERR .NE. 0 ) RETURN
         ELSE
            CALL SLZCOPY (LNI,NI,NI,1,V(1,I),BX(1,NBX))
         END IF
C
   10 CONTINUE
C
...
C
      IF ( NXOUT .GT. 0 ) THEN
         IF ( NXMAX .GT. 0 ) THEN
            CALL SLZCOPY (LNI,LNI,NI,NXOUT,X(1,NBX+1),U)
         ELSE
            DO 20 I = 1,NXOUT
               J = NBX + I
               CALL SLZIOOP (FHNDL,LCOMM,LRERR,J,NI,U(1,I),'X ','GET')
               IF ( LRERR .NE. 0 ) RETURN
   20       CONTINUE
         END IF
      END IF
