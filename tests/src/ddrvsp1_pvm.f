...
C***********************************************************************
C*                                                                     *
C*    DDRVSP1_PVM is a driver model for the standard eigenvalue        *
C*    problem                                                          *
C*                                                                     *
C*           (A)*(x) - eig*(x) = (0)                                   *
C*                                                                     *
C*    where (A) is a symmetric matrix of dimension N. DDRVSP1_PVM is   *
C*    the parallel, PVM based, version of DRVSP1.                      *
C*                                                                     *
C*    Data for DDRVSP1_PVM is read from 'drvsp1.dat' (see below). The  *
...
C
      IF ( MYINST .EQ. 0 ) THEN
C
         WRITE (*,*) 'How many copies of DDRVSP1_PVM.x?'
         READ  (*,*) NPE
C
...
C
         ...
         RMNDR = MOD(N,NPE)
C
C....... initiate NPE copies of DDRVSP1_PVM.x ...............................
C
...
C
         IF ( NPE .GT. 1 ) THEN
            CALL PVMFSPAWN ('DDRVSP1_PVM.x',0,'*',NPE-1,TID(2),INFO)
            IF ( INFO .NE. NPE-1 ) THEN
               CALL PVMFEXIT (INFO)
               STOP '* Error: PVMFSPAWN failed to spawn DDRVSP1_PVM.x *'
            END IF
         END IF
C
